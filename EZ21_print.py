...
class EZ21:
    ...
    def reset(self):
        ...
        self.active_agent = 1
        print("Starting state: {0}".format(self.state))
        return self.state
    ...
    def step(self, action):
        ...
        print("New state: {0}".format(self.state))
        ...
        if self.state[self.active_agent] > 21 or self.state[self.active_agent] < 1:
            rew = 1 - (self.active_agent * 2)
            print("Player {0} went bust! Reward: {1}".format(self.active_agent, rew))
            return 'Terminal', rew, True
