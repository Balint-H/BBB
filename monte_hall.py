...
class MonteCarlo:
    ...
    def __init__(self, env_in, Q_in=None, n_in=20000, epsilon_in=0.2):
        """
    ...
    def iter_opt(self):
        ...
        for i in range(self.n):
            ...
            for state, act, rew in trace:
                ...
                self.Q[state, act] = [
                    self.get_Q(state, act) + (rew - self.get_Q(state, act)) / self.get_invalpha(state, act),
    ...
    def greedy_pol(self, state_in):
        ...
        print("Chosen action is {0}".format(ch))
        return ch

    def e_pol(self, state_in):
        prob = self.epsilon / len(self.env.actions)
        probs = [prob for _ in self.env.actions ]
        probs[self.greedy_pol(state_in)] += 1 - self.epsilon
